...
public class TariffTests
{
  ...
  public void setUp () 
  {
    ...
    Map<String, Object> appServices =
            Map.of("timeService", timeService, "tariffRepo", tariffRepo);
  ...
  public void testSimpleRate ()
  {
    ...
    assertEquals(-2.42, te.getUsageCharge(now.minusMillis(TimeService.DAY), 20.0, 0.0), 1e-6, "correct charge yesterday");
    assertEquals(-12.1, te.getUsageCharge(now.plusMillis(TimeService.DAY), 100.0, 0.0), 1e-6, "correct charge tomorrow");
    assertEquals(-3.63, te.getUsageCharge(now.minusMillis(TimeService.HOUR), 30.0, 0.0), 1e-6, "correct charge an hour ago");
    assertEquals(-1.21, te.getUsageCharge(now.plusMillis(TimeService.HOUR), 10.0, 0.0), 1e-6, "correct charge an hour from now");
