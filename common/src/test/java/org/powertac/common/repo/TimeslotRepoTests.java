...
public class TimeslotRepoTests
{
  ...
  public void testMakeTimeslotNormal ()
  {
    ...
    Timeslot ts1 = repo.makeTimeslot(Instant.ofEpochMilli(baseTime.toEpochMilli() + TimeService.HOUR));
  ...
  {
    Timeslot ts0 = repo.makeTimeslot(Instant.ofEpochMilli(baseTime.toEpochMilli() + TimeService.HOUR));
  ...
  public void testCurrentTimeslot ()
  {
    Timeslot ts0 = repo.makeTimeslot(baseTime);
    Timeslot ts1 = repo.makeTimeslot(Instant.ofEpochMilli(baseTime.toEpochMilli() + TimeService.HOUR));
    Timeslot ts2 = repo.makeTimeslot(Instant.ofEpochMilli(baseTime.toEpochMilli() + TimeService.HOUR * 2));
    assertEquals(ts0, repo.currentTimeslot(), "ts0 current");
    timeService.setCurrentTime(Instant.ofEpochMilli(baseTime.toEpochMilli() + TimeService.HOUR));
    assertEquals(ts1, repo.currentTimeslot(), "ts1 current");
    timeService.setCurrentTime(Instant.ofEpochMilli(baseTime.toEpochMilli() + TimeService.HOUR * 2));
  ...
  public void testFindOrCreateBySerialNumber ()
  {
    ...
    repo.makeTimeslot(Instant.ofEpochMilli(baseTime.toEpochMilli() + TimeService.HOUR));
